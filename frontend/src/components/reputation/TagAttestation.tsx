...
import { useAttestToTagMutation } from '../../services/apiSlice';
import { Button } from '../ui';
import type { PlayerTagWithAttestations, AttestTagRequest } from '../../types/reputation';

interface TagAttestationProps {
  tag: PlayerTagWithAttestations;
  userId?: string;
}

const TagAttestation: React.FC<TagAttestationProps> = ({ tag, userId }) => {
  const [attestToTag, { isLoading }] = useAttestToTagMutation();

  // Check if user has already attested
  const userAttestation = tag.attestations.find(att => att.attester_id === userId);
  ...
  const handleAttestation = async (type: 'support' | 'dispute' | 'neutral') => {
    ...
    try {
      ...
      console.error('Failed to attest to tag:', error);
    }
  };
  ...
  return (
    <div className="mt-3 p-3 bg-white/5 rounded-lg">
      <h4 className="text-xs font-medium text-white mb-2">Attest to this tag</h4>
      ...
      <div className="flex items-center space-x-2">
        ...
        >
          ...
          <span>Support ({tag.attestation_counts.support})</span>
        </Button>
        ...
        >
          ...
          <span>Dispute ({tag.attestation_counts.dispute})</span>
        </Button>
        ...
        >
          ...
          <span>Neutral ({tag.attestation_counts.neutral})</span>
        </Button>
      </div>
    </div>
  );
